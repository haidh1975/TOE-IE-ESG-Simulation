...
> manuscript.** Tables 2–9, Appendix Tables A2–A4 and Figures 2–4 are all generated by
> this one script from one declared parameter set (`TARGETS`). No number in the paper
> is entered by hand.
>
...
This is a **calibration-based simulation**, not an empirical study. Every number
is an illustrative output of an imposed data-generating structure. Nothing here is
evidence about real SME populations.

**`simulate.py` is the single authoritative source for every number in the
manuscript.** Tables 2–9 and Appendix Tables A2–A4 are generated by this one script.
...
| 2 | **`ESG → FC` was never encoded.** FC depended only on size and age, both independent of ESG, so the true effect was exactly zero and H1c could not hold. | The H1c mechanism is now **encoded ex ante and explicitly** (`FC_ESG = −0.180`), justified by sustainability-linked credit access (Guo et al. 2024). The indirect effect is **bootstrapped directly** (5,000 draws), not inferred. |
...
| Guo et al. (2024) | β(ESG→Substantive) ≈ .11; β(Myopia) ≈ −.082; β(ESG×Myopia) ≈ −.058 |
| Nykanen et al. (2023) | ≈ 50% opportunity-dominant cognitive schemas |
